...
BLOCKED_SOURCES=('ウォーカープラス','WEBザテレビジョン','テレビジョン','週刊女性PRIME','女性自身','スポーツ報知','日刊スポーツ','ORICON NEWS','モデルプレス','まいどなニュース')
...
def relevant(x):
 text=(x.get('title','')+' '+x.get('description','')+' '+x.get('source','')).lower(); src=x.get('source','')
 if any(b.lower() in src.lower() for b in BLOCKED_SOURCES): return False
...
def category(x):
 ...
 t=(str(c)+' '+x.get('headline','')+' '+x.get('summary','')).lower(); rules=[('診療報酬・補助金',('診療報酬','補助金','助成金','加算')),('安全管理',('安全','被ばく','線量')),('一般撮影・FPD',('一般撮影','FPD','マンモグラフィ')),('CT・MRI',('CT','MRI')),('核医学',('核医学','SPECT','PET','MIBG','DAT')),('放射線治療',('放射線治療','リニアック','陽子線','粒子線')),('教育',('教育','国家試験','研修','認定')),('学術・AI',('AI','人工知能','学術','研究','画像診断','読影','STAT')),('制度・職能',('制度','職能','人員','養成','確保','タスクシフト'))]
...
def main():
 ...
 for x in db:
  ...
  src=x.get('source','')
  if any(b.lower() in src.lower() for b in BLOCKED_SOURCES): continue
 ...
 for x in fresh:
  k=norm(x['title']); d=dom(x['url']); official=any(d==z or d.endswith('.'+z) for z in OFFICIAL)
  if not k or k in newtitles or not relevant(x): continue
  newtitles.add(k); added.append({'date':x['published'][:10],'category':category(x),'importance':importance(x,official),'headline':x['title'],'summary':x.get('summary') or x.get('description','')[:180],'why':x.get('why') or '放射線部門の実務への影響を確認する価値があります。','source':x.get('source') or d,'url':x['url']})
